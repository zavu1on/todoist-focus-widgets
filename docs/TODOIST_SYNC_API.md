...
#### Формат дельты: целая задача, а не патч полей

Если у задачи изменилось одно поле (например, <u>content</u> — заголовок), в ответе на инкрементальный синк придёт **весь объект задачи целиком** со всеми текущими полями (id, project_id, priority, due, labels, checked и т.д.), а не `{ id, content }`. Дельта — на уровне **набора** изменившихся сущностей (в `items` попадают только задачи, изменившиеся с прошлого <u>sync_token</u>; неизменившиеся в ответе просто отсутствуют), а не на уровне отдельных полей внутри сущности.

Практическое следствие: применение дельты к локальному кэшу — это **upsert по id** (заменить всю строку целиком), а не point-патч конкретного поля.

Удалённые задачи не пропадают из ответа молча — приходят как полный объект-тombstone с <u>is_deleted</u>`: true`, который тоже нужно обработать (удалить/скрыть локально), а не просто отфильтровать по отсутствию в ответе.

> **Степень уверенности**: не дословная цитата из документации — на странице [Todoist API v1](https://developer.todoist.com/api/v1/) этот момент явно текстом не проговорён (или проговорён в части, которая не отфетчилась целиком при ресёрче). Вывод сделан из исходников `@doist/todoist-sdk`: `SyncResponseSchema` валидирует каждый элемент массива `items` **одной и той же схемой с обязательными (не `.optional()`) полями** — `content`, `priority`, `checked`, `dayOrder`, `childOrder` и др. — независимо от того, `full_sync: true` это или инкрементальный ответ. Будь дельта частичной по полям, эти поля были бы опциональными в схеме. Это стабильное, давно задокументированное в сообществе поведение Sync API (не менялось с версии v8/v9), но при появлении сомнений — стоит проверить эмпирически (переименовать задачу и посмотреть форму инкрементального ответа)
